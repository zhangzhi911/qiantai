    ...
    <div class="container">
      ...
      >
        ...
        <el-table-column label="状态" align="center">
          <template slot-scope="scope">
            <el-tag
              :type="scope.row.userFlag===1?'success':(scope.row.userFlag===2?'':'danger')"
            >{{scope.row.userFlag==1?'用户':scope.row.userFlag==2?'商家':scope.row.userFlag==0?'管理员':'禁用'}}</el-tag>
          </template>
        </el-table-column>

        <el-table-column prop="permiss" label="是否会员"></el-table-column>
...
export default {
  ...
  methods: {
    // 下载表格的方法
    exportData() {
      var url = "http://localhost:8099/downLoadGoodsExcel";
    ...
    flag(row, $index) {
      ...
      if ($index.status == 2) {
        flag(userId).then(res => {
          this.getData();
        });
      } else {
        inflag(userId).then(res => {
          this.getData();
          return;
        });
      }
